...
if (doTransient) then
    ...
    elseif (ARGS.LimexNStages == 1) then
        ...
        fine_integrator_limex:set_domain_disc(domainDiscT)
        fine_integrator_limex:set_solver(nlsolver)
        fine_integrator_limex:set_error_estimator(biotErrorEst)
        fine_integrator_limex:set_dt_min(1e-20)

        coarse_integrator_limex = LimexFactory()
        coarse_integrator_limex:set_domain_disc(domainDiscT)
        coarse_integrator_limex:set_solver(nlsolver)
        coarse_integrator_limex:set_error_estimator(biotErrorEst)
        coarse_integrator_limex:set_tol(1e-1)

        --fine_integrator = SimpleIntegratorFactory()
        --fine_integrator:set_time_stepper(time_stepper)
        --fine_integrator:set_domain_disc(domainDiscT)
        --fine_integrator:set_solver(nlsolver)
        --fine_integrator:set_dt_min(endTime/131072)
        --fine_integrator:set_dt_max(endTime)
        ...
        fine_integrator:set_domain(domainDiscT)

        coarse_integrator = ThetaIntegratorFactory()
        coarse_integrator:set_solver(lsolver)
        coarse_integrator:set_domain(domainDiscT)
