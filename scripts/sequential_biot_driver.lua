...
local ARGS = {
    ...
    LimexNStages = util.GetParamNumber("--limex-num-stages", 1, "number of LIMEX stages q"),
...
if (doTransient) then
    ...
    if ((ARGS.LimexNStages > 0)) then
        ...
        util.SolveNonlinearTimeProblem(u, domainDisc0, nlsolver, myStepCallback0, "PoroElasticityInitial", "ImplEuler", 1, startTime, dt0, dt0, dt0, dtRed);
    ...
    elseif (ARGS.LimexNStages == 1) then
        ...
        limex:set_tolerance(ARGS.LimexTOL)
        ...
        --limexFactory = LimexFactory()
        --limexFactory:set_domain_disc(domainDiscT)
        --limexFactory:set_solver(nlsolver)
        --limexFactory:set_error_estimator(biotErrorEst)
        ...
        function myLuaLimexPostProcess(step, time, currdt)
            ...
            problem:post_processing(usol:clone(), step, time) -- todo uncommment to use
        ...
        --dta = (endTime-startTime)/4

        --time_stepper  =  LinearImplicitEuler(domainDiscT)
        --solver = nlsolver
        --derivative
        -- banach_space default space
        --integrator = SimpleTimeIntegrator(time_stepper )
        --integrator:set_time_step(2*dta)
        --integrator:set_dt_min(2*dta)
        --integrator:set_dt_max(2*dta)
        --integrator:set_reduction_factor(0.0)
        --integrator:set_solver(solver)
        --integrator:set_derivative(u:clone())
        -- integrator.set_banach_space(banach_space)


        u_coarse = u:clone()
        --u_fine = u:clone()
        --print("COARSE AB")
        --integrator:apply(u_coarse, startTime+2*dta, u_coarse, startTime+0*dta)
        --print(integrator:get_consistency_error())
        --print("COARSE CD")
        --integrator:apply(u_coarse, startTime+4*dta, u_coarse, startTime+2*dta)
        --print(integrator:get_consistency_error())
        --print("FINE A")
        --integrator:apply(u_fine, startTime+1*dta, u_fine, startTime+0*dta)
        --print(integrator:get_consistency_error())
        --print("FINE B")
        --integrator:apply(u_fine, startTime+2*dta, u_fine, startTime+1*dta)
        --print(integrator:get_consistency_error())
        --print("FINE C")
        --integrator:apply(u_fine, startTime+3*dta, u_fine, startTime+2*dta)
        --print(integrator:get_consistency_error())
        --print("FINE D")
        -- integrator:apply(u_fine, startTime+4*dta, u_fine, startTime+3*dta)
        -- print(integrator:get_consistency_error())
        --print("FINE Ended")
        time = BraidTimer()
        time:start()
        limex:apply(u_coarse, endTime, u_coarse, startTime)
        time:stop()
