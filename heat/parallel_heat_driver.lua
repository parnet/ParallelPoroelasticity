...
braid_desc = {
    type = "integrator",
    time = { t_0 = p_startTime, t_end = p_endTime, n = 256},--math.ceil((endTime-startTime)/dt) },
    cfactor = {2,2,2,2,2}, -- 0 finest level,
    --cfactor = 2,
    default_cfactor = 2,
    max_level = 50,--XARGS.p_max_level,
    ...
    conv_check = {
        max_iter = 50,--2,-XARGS.p_max_iter,
        -- reduction = 1e-9
        absolute = 1e-9
    },
...
if (doTransient) then
    ...
    elseif (ARGS.LimexNStages == 1) then
        ...
        fine_integrator = ThetaIntegratorFactory()
        fine_integrator:set_solver(lsolver)
        fine_integrator:set_domain(domainDisc)

        coarse_integrator = ThetaIntegratorFactory()
        coarse_integrator:set_solver(lsolver)
        coarse_integrator:set_domain(domainDisc)
