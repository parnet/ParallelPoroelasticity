...
XARGS = {
    ...
    p_max_iter = util.GetParamNumber("--maxiter", 10, " maximum number of iterations"),
...
local dim = util.GetParamNumber("-dim", 2, "simulated time frame in seconds")
p_gridName = util.GetParam("-grid", "grids/cube_" .. dim .. "d.ugx", "filename of underlying grid")

local p_startTime = util.GetParamNumber("--start", 0.0, "end time")
local p_endTime = util.GetParamNumber("--end", 4 * math_pi, "end time") -- override by characteristic time
local dtFrac = util.GetParamNumber("--dtFrac", 1e-5, "time step size")
alpha = util.GetParamNumber("--alpha", 0.1, "time step size")
...
local solFileName = "p" .. num_world_ranks .. "s" .. XARGS.num_spatial_procs .. "_sin2" .. dim
...
local p_numRefs = util.GetParamNumber("--num-refs", 3, "total number of refinements (incl. pre-Refinements)") --4 --
local p_N = util.GetParamNumber("-N", 16384, "simulated time frame in seconds")
...
boundary = 0
analyticsolution = "sinAnalyticSolution2d"
...
-- start values
ustart = GridFunction(approxSpace)
ustart:set(0.0)
Interpolate(analyticsolution, ustart, "t", "Inner", 0)
domainDisc:adjust_solution(ustart)

print("Integrate from " .. p_startTime .. " to " .. p_endTime)
...
--cmpConvCheck:set_component_check("x", p0 * 1e-14, 1e-6)
...
if (doTransient) then
    ...
    print("Integrating from 0.0 to " .. p_endTime)
    ...
    if ((ARGS.LimexNStages > 0)) then
        ...
        util.SolveNonlinearTimeProblem(ustart, domainDisc, nlsolver, myStepCallback0, "PoroElasticityInitial", "ImplEuler", 1, p_startTime, dt0, dt0, dt0, dtRed);
    ...
    elseif (ARGS.LimexNStages == 1) then
        ...
        --time_stepper  =  LinearImplicitEuler(domainDisc)
        time_stepper = ThetaTimeStep(domainDisc)
        -- integrator = SimpleTimeIntegrator(time_stepper )
        -- integrator:set_solver(nlsolver)
        -- integrator:set_dt_min(endTime/131072)
        ...
        fine_integrator_limex = LimexFactory()
        fine_integrator_limex:set_domain_disc(domainDisc)
        fine_integrator_limex:set_solver(nlsolver)
        --fine_integrator_limex:set_error_estimator(biotErrorEst)

        fine_integrator = SimpleIntegratorFactory()
        --fine_integrator:set_time_stepper(time_stepper)
        fine_integrator:set_domain_disc(domainDisc)
        fine_integrator:set_solver(nlsolver)
        fine_integrator:set_dt_min(p_endTime / 131072)
        fine_integrator:set_dt_max(p_endTime)
        fine_integrator:set_reduction_factor(0.0)

        coarse_integrator = SimpleIntegratorFactory()
        --coarse_integrator:set_time_stepper(time_stepper)
        coarse_integrator:set_domain_disc(domainDisc)
        coarse_integrator:set_solver(nlsolver)
        coarse_integrator:set_dt_min(p_endTime / 131072)
        coarse_integrator:set_dt_max(p_endTime)
        ...
        t = p_startTime
        i = 0
        dt = (p_endTime - p_startTime) / braid_desc.time.n
        for i = 0, braid_desc.time.n do
            tdisc = LinearImplicitEuler(domainDisc)
            tdisc:disable_matrix_cache()

            step = SimpleTimeIntegrator(tdisc)
            step:set_solver(nlsolver)
            step:set_time_step(dt)
            step:set_dt_max(dt)
            step:set_dt_min(dt)
            ...
            uout = u:clone()
            vtk:print("file", uout, i, t)
        end

        -- braid = xbraid_util.CreateBraidIntegrator(braid_desc,space_time_communicator,logging,fine_integrator, fine_integrator, vtk_scriptor)
        v = ustart:clone()

        --sv_init = StartValueInitializer() -- todo move to desc
        --sv_init:set_start_vector(ustart)
...
--        time:start()
        ...
        --braid:apply(ustart, p_endTime, ustart, p_startTime)
        --time:stop()
